...
def format_streaming_update(update: Dict[str, Any]) -> str:
    ...
    if agent == "search":
        ...
        if "strategy" in update:
            ...
            if hasattr(response, 'data') and response.data:
                ...
                time_taken = getattr(response, 'processing_time', 0)
                return f"✅ **Search Complete** - Found **{candidates} candidates** in {time_taken:.1f}s\n\n*Moving to evaluation phase...*"
        ...
        if update_type == "profile_start":
            ...
            return f"⚖️ **Adjudicator Agent** - Starting evaluation\n\n{thinking_block}"
            ...
            return f"⚖️ **Adjudicator Agent** - Evaluation complete\n\n{thinking_block}"
            ...
            return f"⚖️ **Adjudicator Agent** - AI reasoning in progress...\n\n{thinking_block}"
            ...
            if hasattr(response, 'data') and response.data:
                ...
                time_taken = getattr(response, 'processing_time', 0)
                return f"✅ **Adjudication Complete** - **{relevant}/{total} relevant** profiles in {time_taken:.1f}s\n\n*Generating summary...*"
        
        else:
            return f"⚖️ **Adjudicator Agent** - {update.get('message', 'Processing...')}"
        ...
        if "progress" in update:
            ...
            if hasattr(response, 'data') and response.data:
                time_taken = getattr(response, 'processing_time', 0)
                return f"✅ **Summary Complete** - Analysis generated in {time_taken:.1f}s\n\n*Preparing final results...*"
    ...
    # Handle workflow completion and error updates (same as before)
        ...
        total_time = update.get("total_processing_time", 0)
...
async def enhanced_workflow_streaming(query: str, max_results: int = 7) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        total_time = search_response.processing_time + adj_response.processing_time + sum_response.processing_time
        ...
        try:
            ...
            if "overloaded" in error_str.lower() or "529" in error_str:
                if attempt < max_retries - 1:
                    yield f"⚠️ **API Overloaded** - Retrying in {retry_delay} seconds... (Attempt {attempt + 1}/{max_retries})"
...
def create_streaming_interface():
    ...
    if os.path.exists(logo_path):
        with open(logo_path, 'r') as f:
            logo_content = f.read()
            logo_html = f"""
            <div style="display: flex; justify-content: center; align-items: center; padding: 20px 0; background: linear-gradient(135deg, #2b5556 0%, #21908f 100%); margin-bottom: 20px; border-radius: 15px; box-shadow: 0 8px 32px rgba(43, 85, 86, 0.15);">
    ...
    interface = gr.ChatInterface(
        fn=chat_response,
        type="messages",
        title=f"{logo_html}<div class='app-header'><h1 class='app-title'>Growbal Intelligence v3</h1><p class='app-description'>AI-powered service provider search with real-time streaming evaluation</p></div>",
        description="",
        examples=[
        ...
        css=css,
        textbox=gr.Textbox(
            placeholder="🔍 Ask me about service providers and watch the AI reasoning stream in real-time...",
            container=False,
            scale=7,
            lines=2
        ),
