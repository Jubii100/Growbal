"""
LangGraph workflow implementation for the Growbal Intelligence system.
...
class GrowbalIntelligenceWorkflow:
    ...
    Uses LangGraph to manage the flow between agents.
    """
...
Please feel free to modify your search or ask for something else.""",
    ...
    async def run(self, query: str, max_results: int = 5) -> WorkflowState:
        """
    ...
    def run_sync(self, query: str, max_results: int = 5) -> WorkflowState:
        """
        Synchronous version of the run method.
        """
        import asyncio
        return asyncio.run(self.run(query, max_results))