"""
ORCHESTRATOR-ENABLED FastAPI application with CLEAN STREAMING
...
- Maintains all existing functionality from main_app_5.py
"""
...
# Create FastAPI app
app = FastAPI(
    title="Growbal Intelligence Platform - Orchestrator v6",
    description="AI-powered service provider search with orchestrator and clean streaming",
    version="6.0.0"
)
...
# Copy all existing endpoints from main_app_5.py
...
async def country_selection_page():
    """Serve country selection page - same as main_app_5.py"""
    ...
    <body>
        <div class="container">
            ...
            <div class="app-header">
                <h1 class="app-title">Growbal Intelligence <span class="orchestrator-badge">v6.0 Clean Stream</span></h1>
                <p class="app-description">AI-powered service provider search with clean streaming</p>
            </div>
    ...
    <body>
        <div class="container">
            <div class="session-info">
                ...
                <span class="orchestrator-badge">CLEAN STREAM</span>
        ...
        <script>
            console.log('✅ Clean Stream Chat interface loaded with:');
...
def create_orchestrator_chat_interface():
    """Create chat interface with orchestrator and clean streaming"""
    
    def orchestrator_chat_wrapper(message: str, history: list, request: gr.Request):
        """Orchestrator-enabled chat wrapper with clean streaming"""
        session_id = "unknown"
        country = "unknown"
        ...
        # Extract parameters (same as main_app_5.py)
...
3. Query: {analysis.get('summary', message)}"""
    ...
    # Same styling as main_app_5.py but with clean stream branding
    logo_path = os.path.join(os.path.dirname(__file__), "growbal_logoheader.svg")
    logo_html = ""
    if os.path.exists(logo_path):
        with open(logo_path, 'r') as f:
    ...
    interface = gr.ChatInterface(
        ...
        type="messages",
        title=f"{logo_html}<div class='app-header'><h1 class='app-title'>Growbal Intelligence <span class='orchestrator-badge'>CLEAN STREAM</span></h1><p class='app-description'>AI-powered service provider search with clean streaming</p></div>",
        examples=[
...
print("🧠 Setting up CLEAN STREAM chat interface with orchestrator...")
...
print("✅ Clean stream chat app mounted at /chat-gradio")

# Same health check and debug endpoints as main_app_5.py
@app.get("/health")
async def health_check():
    """Health check endpoint"""
    return {
        "status": "healthy", 
        "service": "growbal-intelligence-orchestrator-clean-stream",
        "version": "6.0.0",
        "features": ["orchestrator", "clean_streaming", "tool_routing"],
...
if __name__ == "__main__":
    ...
    print("🚀 Starting ORCHESTRATOR-ENABLED Growbal Intelligence FastAPI application with CLEAN STREAMING...")
    ...
    print()
    print("🧠 NEW CLEAN STREAMING FEATURES:")
    print("   ✅ Intelligent tool routing based on message analysis")
    print("   ✅ Clean streaming: shows only current step + final result")
    print("   ✅ No accumulated agentic decision-making history")
    print("   ✅ Maintains user/bot conversation history")
    print("   ✅ Real-time status updates (temporary)")
    print("   ✅ Final result display (persistent)")
