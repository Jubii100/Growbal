...
class ChecklistCustomizationEngine:
    ...
    async def research_industry_standards(
        ...
        # Extract checklist modifications with timeout protection
        print("  Extracting checklist modifications via LLM")
        try:
            modifications = await self._extract_modifications(processed_content, profile_text)
            print(f"  Successfully extracted {len(modifications.get('mandatory_additions', []))} mandatory and {len(modifications.get('recommended_additions', []))} recommended modifications")
        except Exception as e:
            print(f"  Error extracting modifications: {str(e)}")
            # Provide fallback empty modifications to prevent workflow hang
            modifications = {
                'mandatory_additions': [],
    ...
    async def _research_single_query(self, query: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            search_results = await self.web_search.search(query_text, num_results=2)
            
            if not search_results:
                print(f"  No search results for: {query_text}")
            ...
            ranked = sorted(search_results, key=lambda r: lex(r.get('title', '')), reverse=True)[:3]
            print(f"  Found {len(ranked)} results to fetch")
            
            # Fetch and parse with limits
            fetch_tasks = [self.fetcher.fetch(r.get('link', '')) for r in ranked if r.get('link')]
            if not fetch_tasks:
                print(f"  No valid URLs to fetch for: {query_text}")
                result['error'] = 'No valid URLs found'
                return result
                
            fetched_pages = await asyncio.gather(*fetch_tasks, return_exceptions=True)
            
            for i, (r, page) in enumerate(zip(ranked, fetched_pages)):
                if isinstance(page, Exception):
                    print(f"  Fetch error for {r.get('link', '')}: {str(page)}")
                    continue
                    
                if not getattr(page, 'html', None):
                    print(f"  No HTML content from: {r.get('link', '')}")
                    continue
                    
                parsed = self.parser.extract_business_info(page.html, r.get('link', ''))
                content = parsed.get('business_content', '')
                
                if content:
                    cleaned_content = self.cleaner.clean_and_extract(content) or content
                    result['pages'].append({
                        'title': r.get('title', ''),
                        'url': r.get('link', ''),
                        'content': cleaned_content,
                    })
                    print(f"  Extracted content from: {r.get('link', '')}")
                else:
                    print(f"  No business content from: {r.get('link', '')}")
            
            result['success'] = len(result['pages']) > 0
            print(f"  Successfully processed {len(result['pages'])} pages for query: {query_text}")
            
        except Exception as e:
            print(f"  Error researching query '{query_text}': {str(e)}")
            result['error'] = str(e)
...
class AnswerGatheringEngine:
    ...
    async def research_answers(
        ...
        """Research answers for specific checklist items using existing research content"""
        ...
        # Use the provided research content instead of generating new searches
        if not research_content:
            print("  No research content provided, skipping answer extraction")
            return {
                'phase': ResearchPhase.ANSWER_GATHERING,
        ...
        # Create parsed content structure from the research content
        parsed_content = [{
            'query': {'item_key': 'research_content_general', 'text': 'general_research'},
            'url': 'research_content',
            'title': 'Research Content',
            'business_info': {'business_content': research_content},
            'relevance_score': 1.0
        }]
        
        # Extract specific answers using the research content
        answers = await self._extract_answers_from_research_content(research_content, pending_items)
        
        return {
            'phase': ResearchPhase.ANSWER_GATHERING,
            'provider_info': provider_info,
            'extracted_answers': answers,
            'research_results': [],  # No search results since we're using existing content
    ...
    async def _parse_search_results(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        max_fetches = 6  # Limit total fetches to prevent infinite loops
        fetch_count = 0

        for search_result in search_results:
            if fetch_count >= max_fetches:
                print(f"  Reached max fetch limit ({max_fetches}), stopping to prevent infinite loops")
                break
            ...
            if not results:
                print(f"  No search results for query: {query.get('text', '')}")
                continue
            ...
            if not reranked:
                print(f"  No reranked results for query: {query.get('text', '')}")
                continue
            ...
            for item in reranked:
                ...
                try:
                    fetch_count += 1
                    print(f"  🌐 Fetching ({fetch_count}/{max_fetches}): {url}")
                    fetched = await self.fetcher.fetch(url)
                    
                    if not getattr(fetched, 'html', None):
                        print(f"  No HTML content from: {url}")
                        continue
                        
                    business_info = self.parser.extract_business_info(fetched.html, url)
                    
                    if not business_info.get('business_content'):
                        print(f"  No business content extracted from: {url}")
                        continue
        ...
        print(f"  Total parsed results: {len(parsed_results)}")
    ...
    async def _extract_answers(
        ...
        for item in checklist_items:
            ...
            if item_key in content_by_item:
                ...
                # Compile content for LLM with aggressive truncation
                content_text = "\n".join([
                    f"Content: {c['business_info'].get('business_content', '')[:300]}"  # Limit each content piece
                    for c in relevant_content[:2]  # Limit to 2 most relevant pieces
                ])
    ...
    async def _extract_answers_from_research_content(
        self,
        research_content: str,
        checklist_items: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Extract specific answers from the research content for checklist items"""
        answers = {}
        
        # Extract answer for each item using the research content
        for item in checklist_items:
            item_key = item['key']
            
            # Use the research content directly for answer extraction
            answer = await self.llm.extract_answer_from_content(
                research_content[:2000],  # Limit content size for LLM processing
                item['prompt']
            )
