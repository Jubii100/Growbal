...
class OnboardingLLM:
    ...
    def __init__(self, model_name: str = "gpt-5", temperature: float = 0.0, token_limit: int = 30000, timeout: int = 60, max_retries: int = 2):
    ...
    def generate_initial_checklist(self, profile_text: str) -> List[Dict[str, Any]]:
        """Generate the initial onboarding checklist as a list of items.

        Returns a list of checklist item dicts with keys:
        key, prompt, required, status, value
        """
        checklist, _ = self.generate_initial_checklist_with_research(profile_text)
        return checklist
    ...
    def _gather_research_content(self, profile_text: str) -> str:
        ...
        for q in queries[:3]:
            try:
                ...
                results = web._search_sync(query_text, num_results=5)
            except Exception as e:
            ...
            for r in reranked:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    self._append_to_web_log(f"FETCH url='{url}' render_js=True timeout=30")
                    page = fetcher._fetch_sync(url, render_js=True, timeout=30)
                    ...
                    self._append_to_web_log(f"ERROR fetch/parse url='{url}'")
                    continue

            if len(aggregated_snippets) >= 4:
                break
        ...
        if not aggregated_snippets and all_search_results:
            try:
                ...
                if seed:
                    ...
                    for u in candidates:
                        try:
                            self._append_to_web_log(f"FALLBACK FETCH url='{u}' render_js=True timeout=30")
                            page = fetcher._fetch_sync(u, render_js=True, timeout=30)
                            status = getattr(page, "status_code", None)
                            html = getattr(page, "html", "")
    ...
    async def generate_clarifying_question(
        ...
        if isinstance(research_context, dict) and research_context:
            ...
            research_text = research_context[:400]
        ...
        try:
            result_msg = await (prompt | structured_llm).ainvoke(inputs)
        except Exception as e:
            try:
    ...
    async def extract_checklist_modifications(
        ...
        template = (
            "You analyze industry research to suggest onboarding checklist modifications for providers.\n"
            "Location: {location}\n\n"
            "Research Content (may be truncated):\n{content}\n\n"
            "Return JSON with keys: mandatory_additions, recommended_additions, items_to_remove."
        )
        prompt = ChatPromptTemplate.from_template(template)
        ...
        # Add a thin retry around async calls
        result = None
        last_err: Optional[Exception] = None
        for _ in range(1 + 1):
            try:
    ...
    async def extract_answer_from_content(
        ...
        template = (
            ...
            "Return strict JSON with keys: value, confidence, source, evidence."
        )
        prompt = ChatPromptTemplate.from_template(template)
    ...
    def rerank_results_by_title(
        ...
        # Prepare LLM input
        template_str = self._read_prompt(self._rerank_by_title_prompt_path)
        prompt = ChatPromptTemplate.from_template(template_str)
        structured_llm = self.chat.with_structured_output(RerankResponse, method="function_calling")
        chain = prompt | structured_llm
        try:
            payload = {
                "query": query,
            ...
            payload, formatted_text = self._fit_inputs_to_limit(prompt, payload, keys_to_truncate=["results", "query"])
            ranked_model = chain.invoke(payload)
            ranked = ranked_model.model_dump()
            self._log_interaction("rerank_results_by_title", formatted_text, ranked)
            order = ranked.get("ordered_urls", [])
            url_to_item = {r.get("url"): r for r in baseline}
            ordered = [url_to_item[u] for u in order if u in url_to_item]
            # Append any missing
            seen = set(order)
            ordered.extend([r for r in baseline if r.get("url") not in seen])
        except Exception as e:
            try:
                self._log_llm_error("rerank_results_by_title", e, self._format_prompt(prompt, payload))
            except Exception:
                pass
