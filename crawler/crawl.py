...
class ServiceProviderOutput(BaseModel):
    ...
    service_tags: List[str] = Field(description="Do your due diligence on summarising the service tags based on a holistic consideration of the scattered relevant service info throughout the entirety of the given CLEAN HTML content. Empty list if unavailable.")
...
def check_search_relevance(search_term: str, search_results: Dict[str, Any]) -> RelevanceCheckOutput:
    ...
    prompt_template = ChatPromptTemplate.from_messages([
        ("system", prompt),
        ("human", f"Check relevance of search results to the given search term (but most importantly make sure it is a company's official website, not a search facilitator website or service): {search_term}")
    ])
...
async def process_relevant_links(relevant_links):
            ...
            try:
                ...
                for i, link in enumerate(result.about_us_link):
                    ...
                    try:
                        ...
                        elif clean_html_sum and (token_count := count_tokens(clean_html_sum)) < num_tokens_allowed:
                            print(token_count)
                ...
                if clean_html_sum != "":
                    ...
                    result = call_llm(prompt_template = prompt_template, input_dict = {"html_content": clean_html_sum}, structured_output = ServiceProviderOutput, temperature=0)
                ...
                if hasattr(result, 'name') and result.name and result.name != "":
                    if hasattr(result, 'name') and result.name and result.name != "":
                        user = None
                        if result.website and validate_url(result.website):
                        ...
                        if user:
                            ...
                            service_data_json = {
                                ...
                                "service_tags": result.service_tags if result.service_tags else [],
...
async def main(env_path: str, search_starters: list[str]) -> None:
    ...
    for industry_service_country_region in search_starters:
        try:
            prompt_template = ChatPromptTemplate.from_messages([
                ("system", prompt),
                ("human", f"Generate search terms for the industry/service & country/region: {industry_service_country_region}")
            ])

            # llm = ChatOpenAI(openai_api_key=os.environ["OPENAI_API_KEY"], model=llm_model, temperature=0).with_structured_output(SearchTermsOutput)
            # llm = ChatAnthropic(
            #     model="claude-3-5-sonnet-20241022",
            #     anthropic_api_key=os.environ["ANTHROPIC_API_KEY"], temperature=0).with_structured_output(SearchTermsOutput)
            # llm_chain = prompt_template | llm
            # llm_chain = llm_chain.with_retry(
            #     retry_if_exception_type=(OverloadedError,),
            #     wait_exponential_jitter=True,    
            ...
            result = call_llm(prompt_template = prompt_template, input_dict = {"industry_service_country_region": industry_service_country_region}, structured_output = SearchTermsOutput, temperature=0)
            ...
            for search_term in result.search_terms:
                ...
                await process_relevant_links(relevant_links)
        except Exception as e:
            error_message = f"Error processing service {industry_service_country_region}: {e}"
            print(error_message)
            with open(os.environ["LOG_FILE_PATH"], "a") as log_file:
                log_file.write(error_message + "\n")
                # traceback.print_exc(file=log_file)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the crawl with an .env file and services.")
    ...
    parser.add_argument(
        "services_file",
        help="Path to file containing service names, one per line."
    )

    args = parser.parse_args()

    with open(args.services_file, 'r') as f:
        industry_service_country_region_list = [line.strip() for line in f if line.strip()]

    asyncio.run(main(args.env_path, industry_service_country_region_list))